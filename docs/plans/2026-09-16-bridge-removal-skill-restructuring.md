# Plan: Bridge Removal & Skill Directory Restructuring

**Status:** Completed (Skill restructuring deferred per Captain direction)  
**Date:** 2026-09-16  
**Author:** Antigravity Control Plane (First Mate)  
**Tracking Issue / Context:** User request to remove legacy bridge infrastructure (preserving only `adopt.sh` relocated to `scripts/adopt.sh`), migrate nested sub-skills (`design/impeccable`, `design/interface-design`, `frameworks/laravel-projects/filament`, `frameworks/laravel-projects/laravel`) into standardized `references/` directories, and synchronize all affected skills, rules, and documentation.
...
> 2. Removing the legacy bridge directory entirely.
...
1. **Retain & Relocate `adopt.sh`:**
   ...
   - Moved `adopt.sh` to `scripts/adopt.sh` with executable permissions (`chmod +x`).
   - Updated `adopt.sh` internal header, help messages, boundary check exclusions (`scripts/`), and fail-closed gates.
   - Completely removed the legacy bridge directory (`config-reader.py`, `dispatch.sh`, `session-runner.sh`, `README.md`, and test sessions).
   - Removed legacy sessions from `.gitignore`.
...
3. **Purged Bridge & Re-Anchored Constitution to Native Subagents:**
   - In `AGENTS.md` and `.agents/rules/agent-control-plane.md`, removed deprecated Cross-Harness Bridge sections (`dispatch.sh`) and Cross-Project Session Protocol (`session-runner.sh`).
...
### Phase 1: Script Relocation & Bridge Removal (`SHIP`)
- [x] **Task 1.1: Relocate `adopt.sh` to `scripts/adopt.sh`**
  - Create directory `scripts/`.
  - Move `adopt.sh` to `scripts/adopt.sh`.
  - Update script usage banner, comments, and boundary checks (disallow transferring `scripts/` to target repos, audit target for 0 symlinks, ensure `test ! -e "$TARGET/scripts"`).
  - Verify syntax with `bash -n scripts/adopt.sh` and test with `bash scripts/adopt.sh --help`.
- [x] **Task 1.2: Remove legacy bridge directory and update `.gitignore`**
  - Delete legacy bridge tree.
  - Update `.gitignore` to remove legacy sessions.
...
- [x] **Task 4.1: Update `AGENTS.md` and `.agents/rules/agent-control-plane.md`**
  - Update references to `./scripts/adopt.sh`.
  - Remove deprecated Cross-Harness Bridge sections (`dispatch.sh`, `session-runner.sh`) and Cross-Project Session Protocol, re-anchoring on native `invoke_subagent`.
  - Ensure `.agents/rules/agent-control-plane.md` is updated identically.
- [x] **Task 4.2: Update `productivity/adopt-acon/SKILL.md`**
  - Change all adoption commands and references to `./scripts/adopt.sh`.
  - Update architecture diagrams and verification checklist to verify `test ! -e "$TARGET/scripts"`.
- [x] **Task 4.3: Update `README.md` and `.agents/INDEX.md`**
  ...
  - Update `.agents/INDEX.md` to remove lookup entries for legacy components, and update adoption reference to `scripts/adopt.sh`.
...
- [x] **Task 5.1: Integrity & Broken Link Audit**
  ...
  - Confirm legacy bridge directory no longer exists on disk.
  - Run audit across `AGENTS.md`, `.agents/`, `README.md`, `scripts/` to confirm zero unwanted stale references remain.
