...
- **The Machine Contract & Zero-Preamble Principle (Token & Velocity Discipline):**  
  ...
  2. *Schema-Locked Envelopes:* Subagents must report progress, inventories, and diffs strictly inside the canonical YAML schemas in `.agents/reference/schemas/` (`scout-report.yaml`, `ship-diff.yaml`, `handoff-state.yaml`, `task-contract.yaml`, `decision-sheet.yaml`).
