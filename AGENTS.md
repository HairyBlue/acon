...
- **Adapter Layer**: When running external or cross-harness background tasks, workers are executed via [`.agents/adapters/dispatch.sh`](.agents/adapters/dispatch.sh) and [`.agents/adapters/session-runner.sh`](.agents/adapters/session-runner.sh).
...
5. **Local Adoption Companion**:  
   [`adopt.sh`](.agents/adapters/adopt.sh) remains the canonical tool if the Captain wants to permanently adopt ACON directly into that target repository.
6. **Governed by `acon.yaml`**:  
   All session routing, harness resolution, and model exclusions adhere strictly to [`acon.yaml`](acon.yaml).
